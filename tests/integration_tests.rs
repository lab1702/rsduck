...
async fn test_readonly_protection() {
    ...
    assert!(body["error"]["message"].as_str().unwrap().contains("read-only mode"));
}
...
async fn test_missing_sql_parameter() {
    ...
    let body: Value = response.json();
    assert_eq!(body["success"], false);
    assert!(body["error"]["message"].as_str().unwrap().contains("Missing 'sql' parameter"));
}

fn create_test_app(state: AppState) -> axum::Router {
    use axum::routing::{get, post};
    use rsduck::{health_check, execute_query_post, execute_query_get, execute_command_post, execute_command_get};
